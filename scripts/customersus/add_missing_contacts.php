...
//	$leads = $api->find('leads', ['limit_rows' => $count, 'limit_offset' => $offset]);
	...
	// Получение сделок (метод find класса Api_Client возвращает почему-то удаленные сделки как актуальные)
...
//			var_dump($contacts);
		foreach ($contacts as $contact) {
//			$user_id = 0;
			foreach ($contacts['custom_fields'] as $field) {
	...
	// формирование массива элементов вида account_id => [users_ids] по результату запоса в базу
	$db_result = [];
...
//	var_dump($diffs);
	...
	foreach ($diffs as $account_id => $users_ids) {
		$contacts = [];
		$lead = array_search($account_id, $by_account);
		foreach ($users_ids as $id) {
			$contacts[] = array_search($by_user, $id);
		}
	}
